...
const Faculty = () => {
  ...
  useEffect(() => {
    if (!facultyData.length) {
      setFilteredData([]); // Ensure no data is displayed
      return;
    }

    let filtered = facultyData.filter((item) => {
      const searchableFields = ["name", "faculty_name", "title", "department", "award_name", "conference_name", "program_name", "patent_title", "journal_name"];
    ...
    setFilteredData(filtered.length ? filtered : []); // Show 0 results when filtered data is empty
  }, [searchQuery, startDate, endDate, facultyData]);
